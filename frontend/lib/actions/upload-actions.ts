...
export async function getPresignedURLAction(
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error(
        error.error || `Failed to get presigned URL: ${response.status}`,
      );
    }
...
// Note: For file downloads, use getFileDownloadAction from file-actions.ts
// which uses the file ID instead of the S3 key
